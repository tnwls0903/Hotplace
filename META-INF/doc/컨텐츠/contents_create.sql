...
COMMENT ON TABLE contents is '컨텐츠 - 핫한 장소 추천';
...
INSERT INTO contents(contentsno, memberno, categoryno, title, content, recom, cnt, replycnt, passwd, 
                     word, rdate, file1, file1saved, thumb1, size1)
VALUES(contents_seq.nextval, 1, 1, '서울 유명 데이트 장소', '여의도 벚꽃 축제', 0, 0, 0, '123',
       '데이트', sysdate, 'space.jpg', 'space_1.jpg', 'space_t.jpg', 1000);

-- 유형 1 전체 목록
SELECT contentsno, memberno, categoryno, title, content, recom, cnt, replycnt, passwd, word, rdate,
...
DELETE FROM contents
WHERE categoryno=12 AND contentsno <= 41;
...
-- ----------------------------------------------------------------------------------------------------
-- 모든글
SELECT contentsno, memberno, categoryno, title, content, recom, cnt, replycnt, word, rdate,
       file1, file1saved, thumb1, size1, map, youtube
FROM contents
ORDER BY contentsno ASC;
...
SELECT contentsno, memberno, categoryno, title, content, recom, cnt, replycnt, word, rdate,
           file1, file1saved, thumb1, size1, map, youtube
FROM contents
WHERE categoryno=8 AND word LIKE '%부대찌게%'
ORDER BY contentsno DESC;
...
SELECT contentsno, memberno, categoryno, title, content, recom, cnt, replycnt, word, rdate,
           file1, file1saved, thumb1, size1, map, youtube
FROM contents
WHERE categoryno=8 AND (title LIKE '%부대찌게%' OR content LIKE '%부대찌게%' OR word LIKE '%부대찌게%')
ORDER BY contentsno DESC;
...
SELECT contentsno, title, word
FROM contents
WHERE categoryno=8 AND (LOWER(word) LIKE '%food%'); -- 대소문자를 일치 시켜서 검색

SELECT contentsno, title, word
FROM contents
WHERE categoryno=8 AND (UPPER(word) LIKE '%' || UPPER('FOOD') || '%'); -- 대소문자를 일치 시켜서 검색 ★

SELECT contentsno, title, word
FROM contents
WHERE categoryno=8 AND (LOWER(word) LIKE '%' || LOWER('Food') || '%'); -- 대소문자를 일치 시켜서 검색
...
-- ----------------------------------------------------------------------------------------------------
-- step 1
SELECT contentsno, memberno, categoryno, title, content, recom, cnt, replycnt, rdate,
           file1, file1saved, thumb1, size1, map, youtube
FROM contents
WHERE categoryno=1 AND (title LIKE '%단풍%' OR content LIKE '%단풍%' OR word LIKE '%단풍%')
ORDER BY contentsno DESC;
...
FROM (
           ...
           FROM (
                     SELECT contentsno, memberno, categoryno, title, content, recom, cnt, replycnt, rdate,
                                file1, file1saved, thumb1, size1, map, youtube
                     FROM contents
                     WHERE categoryno=1 AND (UPPER(title) LIKE '%' || UPPER('단풍') || '%' 
...
SELECT COUNT(*) as cnt
FROM contents
WHERE categoryno IN(1,2,3);

-- 다수의 카테고리에 속한 레코드 모두 삭제: IN
SELECT contentsno, memberno, categoryno, title
FROM contents
WHERE categoryno IN(1,2,3);
...
-- ----------------------------------------------------------------------------------------------------
-- category + contents INNER JOIN
-- ----------------------------------------------------------------------------------------------------
-- 모든글
SELECT c.name,
       t.contentsno, t.memberno, t.categoryno, t.title, t.content, t.recom, t.cnt, t.replycnt, t.word, t.rdate,
       t.file1, t.file1saved, t.thumb1, t.size1, t.map, t.youtube
FROM category c, contents t
WHERE c.categoryno = t.categoryno
ORDER BY t.contentsno DESC;

-- contents, member INNER JOIN
SELECT t.contentsno, t.memberno, t.categoryno, t.title, t.content, t.recom, t.cnt, t.replycnt, t.word, t.rdate,
       t.file1, t.file1saved, t.thumb1, t.size1, t.map, t.youtube,
       a.mname
...
FROM (
     SELECT contentsno, memberno, categoryno, title, content, recom, cnt, replycnt, rdate,
            file1, file1saved, thumb1, size1, map, youtube, rownum as r
     FROM vcontents -- View
     WHERE categoryno=14 AND (title LIKE '%야경%' OR content LIKE '%야경%' OR word LIKE '%야경%')
)
WHERE r >= 1 AND r <= 3;
...
-- ----------------------------------------------------------------------------------------------------
SELECT contentsno, memberno, categoryno, title, thumb1, r
FROM (
           SELECT contentsno, memberno, categoryno, title, thumb1, rownum as r
           FROM (
                     SELECT contentsno, memberno, categoryno, title, thumb1
                     FROM contents
                     WHERE categoryno=1
                     ORDER BY recom DESC
           )          
